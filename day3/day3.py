    ...
    for y in range(0, y_total):
        ...
        if current_position in sympols or current_position == "." or y == y_total - 1:
            if len(my_number) > 0:
                ...
                # Had a ton of issues but my original code does work without adding in breaks.
                # Note to kill terminal when trying to figure things out.
                #done = False
                for yy in range(y_start, y_end + 1):
                    for xx in range(x_start, x_end + 1):
                        
                        if data[xx][yy] in sympols:
                            sum += int(my_number)
                            #print(my_number)
                            my_number = ""
                            #done = True
